...
以下我們將以「大學選課系統」為貫串本章的案例。該系統允許老師開課與打成績，並讓學生進行選課。

首先，我們可以從系統功能的角度，用使用案例圖將之描繪出來：
- **邊界**：大學選課系統 (University System)
- **參與角色**：學生 (Student)、老師 (Teacher)
- **使用案例**：開課 (Offer course)、選課 (Take course)、評分 (Score)、查詢成績 (List grade)
...
針對 **選課 (Take Course)** 這個使用案例，我們可以寫出下方的結構化使用案例描述：
...
延續大學選課的案例，我們把焦點放在「課程 (Course)」這個物件上。一門課程從無到有，通常會經歷幾個明顯的狀態變化。我們可以使用狀態圖來表達課程物件的狀態行為。
...
在先前的「使用案例圖」與「狀態圖」中，我們分別描繪了系統功能需求與單一課程物件的狀態變化。接著，如果我們想了解在一個完整情境下（如開課與選課），多個物件（Main, University, Teacher, Student）之間是如何互相呼叫的，我們可以使用**循序圖**來觀察訊息傳遞的順序。

以下是一段操作這些物件的 Java 程式碼：
...
最後，針對學員選課的「流程」，我們也可以使用**活動圖 (Activity Diagram / Flowchart)** 來描繪。活動圖很適合用來表達包含判斷邏輯（如判斷名額是否已滿）的運作過程。
