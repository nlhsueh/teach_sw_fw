...
### 資料驅動設計
將變異的地方存放在外部的檔案中，例如路徑、類別名稱、排版方式或其他設定資料等，系統執行時讀取這些值來**參數的**、動態的調整系統的行為。這樣的設計避免程式設計時的變動次數，也因為把這些資料抽離，降低其對系統的影響。

有時候我們會將企業規則包裝為 SQL stored function，抽離於程式的內部，也是一種 Protected Variation 的體現。
