...
> * **開閉原則 (OCP)**：PV 的目標就是達成 OCP，讓系統對擴充開放，對修改關閉。
> * **資訊隱藏 (Information Hiding)**：將變動的秘密隱藏在介面之後，也是一種防護手法。
> * **封裝與權限控制**：透過 `private` 或 `protected` 限制存取，本身就是為了「保護」程式碼不被隨意修改或產生不當耦合。

### 7.7.1 Order 範例

在這個範例中，我們將觀察一個電商系統中的支付流程。原始設計中，訂單系統僅使用簡單的 `String` 來儲存支付方式，這導致支付處理邏輯必須透過大量的 `if-else` 或 `switch` 來判斷字串內容。這樣的設計使得支付處理器與具體的支付邏輯緊密聯繫在一起，當我們想要增加新的支付方式時，往往需要修改那穩定且核心的支付處理類別，這正是缺乏「防護」的表現。
