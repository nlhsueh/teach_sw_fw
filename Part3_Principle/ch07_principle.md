...
- **偶然內聚性（Coincidental cohesion，內聚力最低）**
  - **說明：** 將完全無關的功能硬塞在同一個模組或類別裡，各部分唯一的相同點只是「剛好被放在一起」。這是最糟糕的設計。
  - **範例：** 存在一個名為 `Utility` 或 `Misc` 的類別，裡面同時包含了 `calculateTax()`（算稅金）、`printDocument()`（印文件）和 `parseXML()`（解析 XML）。這三者毫無關聯。

- **邏輯內聚性（Logical cohesion）**
  - **說明：** 將邏輯上看似同類，但本質與實作完全不同的操作綁在一起。通常會透過傳入一個參數（flag）來決定要執行哪部分的程式碼。
  - **範例：** 一個名為 `InputManager` 的模組，裡面同時負責「讀取鍵盤輸入」、「讀取檔案內容」和「讀取網路封包」。雖然名稱都叫「輸入」，但實作細節與目的差異甚大，不應混在一起。

- **時間性內聚性（Temporal cohesion）**
  - **說明：** 將數個沒有直接因果關係，但剛好需要「在同一個時間點執行」的功能放在一起。
  - **範例：** 一個 `initSystem()`（系統初始化）函式，裡面包含了「連接資料庫」、「渲染使用者介面」及「設定日誌檔案」。這三件事只因為都在系統啟動時發生而被放在一起，更好的作法是分拆由各自獨立的模組負責。

- **程序內聚性（Procedural cohesion）**
  - **說明：** 為了確保一組操作能「依照特定順序執行」而將它們放在一起，不過這些操作之間並沒有直接的資料傳遞。
  - **範例：** 一個名為 `shutdownProcess()` 的函式，裡面依序執行了「寫入離開日誌」、「清空畫布」和「斷開網路連線」。它們必須按順序發生，但彼此互相獨立。

- **依序內聚性（Sequential cohesion）**
  - **說明：** 不僅步驟有先後順序，而且「上一個步驟的輸出結果，剛好是下一個步驟的輸入資料」，就像工廠的生產線一樣。
  - **範例：** 一個名為 `formatAndPrintReport()` 的函式，第一步先「讀取原始數據（輸出字串）」，第二步「將字串轉換為報表格式（輸入字串，輸出報表）」，第三步「列印報表」。

- **功能內聚性（Functional cohesion）**
  - **說明：** 模組內的所有元素都只為了解決「單一個且定義明確的任務」。這是傳統模組化設計中最理想的高內聚力。
  - **範例：** 一個 `calculateCompoundInterest()`（計算複利）的函式，裡面的所有運算都純粹為了計算並回傳複利結果，不參雜額外雜事。

- **物件內聚性（Object cohesion，內聚力最高）**
  - **說明：** 物件導向程式設計的核心。資料屬性與操作這些資料的方法，因為「屬於同一個實體概念」而被封裝在同一個類別中。
  - **範例：** 一個 `BankAccount`（銀行帳戶）類別，裡面包含了提款（`withdraw`）、存款（`deposit`）的方法，並且共同維護與操作內部的餘額（`balance`）狀態。
...
<details>
<summary>解答</summary>
說明：介面只定義了模組應該提供的功能，而不包含具體的實作細節。這樣其他模組依賴的只是抽象的介面而不是具體實作，當具體實作改變時，只要介面不變，就不會影響到依賴它的模組，從而降低了模組之間的耦合力。
</details>
...
<details>
<summary>解答</summary>
說明：時間性內聚性 (Temporal cohesion)。因為這些功能只是剛好在初始化的同一個時間點需要被執行，功能本身的關聯性較弱。
</details>
...
<details>
<summary>解答</summary>
說明：`StringTokenizer` 透過封裝內部狀態（例如目前讀取的位置）來運作，對外僅提供所需的方法如 `hasMoreTokens()` 和 `nextToken()`。我們不能直接存取或修改內部位置的變數，是為了防止外部程式不當修改導致狀態混亂或產生錯誤。這符合**資訊隱藏原則**，不公開多餘的資訊與服務，讓模組的實作細節對外部隱藏，提高系統的安全性和可維護性。
</details>
...
開閉原則（**Open-Closed Principle**的意義即為「擴充程式優於修改程式」。當我們完成一個程式或類別後，日後有很大的可能性要去擴充它的功能，這時候大部分的程式設計師會將原有的程式叫出來修改，修改程式會帶來許多問題：
...
「受保護變異」（Protected Variation）是軟體設計原則之一，其核心概念是：保護客戶端，「當系統的某個部分發生變動或擴充時，不會產生連鎖反應（Ripple Effect）去影響到系統的其他部分」。

* 隔離變化：
    * 該原則旨在將系統中可能發生變化的部分隔離起來，致使其不會影響到其他相對穩定的部分。
    * **與「模組化」的關係**：模組化是將系統拆分，而「防護變異」則是進一步指導我們**在哪裡**拆分——即在那些「易變」與「不變」的交界處建立防火牆。
* 降低耦合：
...
> [!NOTE]
> **與其他原則的關係：**
> PV 常被視為許多設計原則的「根本目的」，而其他原則是達成它的「手段」：
> * **開閉原則 (OCP)**：OCP 講的是如果要變的話，用擴充來變， PV 講的是：把變的部分和不變的部分隔離開來。
> * **資訊隱藏 (Information Hiding)**：將變動的秘密隱藏在介面之後，也是一種防護手法。
> * **封裝與權限控制**：透過 `private` 或 `protected` 限制存取，本身就是為了「保護」程式碼不被隨意修改或產生不當耦合。

### 7.7.1 Order 範例

在這個範例中，我們將觀察一個電商系統中的支付流程。原始設計中，訂單系統僅使用簡單的 `String` 來儲存支付方式，這導致支付處理邏輯必須透過大量的 `if-else` 或 `switch` 來判斷字串內容。這樣的設計使得支付處理器與具體的支付邏輯緊密聯繫在一起，當我們想要增加新的支付方式時，往往需要修改那穩定且核心的支付處理類別（`客戶端`），這正是缺乏「防護」的表現。
...
public class Order {
    ...
    // 這裡回傳的是原始字串，導致後續必須使用 if-else 判斷，缺乏防護
...
1.  **存在問題：**
    * **過度依賴原始型態 (String)**：`Order` 類別使用 `String` 來代表支付方式，導致 `PaymentProcessor` 必須直接處理這些字串並解析具體的支付邏輯。
    * **缺乏防護**：支付方式是「易變」的部分，現在卻與「穩定」的支付處理器直接耦合。當新增支付方式時，必須修改 `PaymentProcessor` 的程式碼，違反了「受保護變異」原則。

2.  **修改方案：**
    * **由字串改為介面**：將支付方式從原始的 `String` 提升為抽象的 `PaymentMethod` 介面。
    * **建立防護牆**：透過介面定義標準動作，讓不同的支付實作隱藏在介面後方，保護 `PaymentProcessor` 不受支付種類增減的影響。
    * 修改後範例如下：

```java
public class Order {
    // 這裡使用了 PaymentMethod 介面，而不是具體字串或類別
    // 這就是「防火牆」，Order 不再直接接觸變動的支付實作
...
當兩個模組溝通的方式是字串時，客戶端與處理端都必須知道這個字串的內容，這會造成耦合。如果我們將溝通的方式改為介面，客戶端與處理端都不需要知道介面背後的具體內容，這會降低耦合。
...
**GRASP** (General Responsibility Assignment Software Patterns) 是由 Craig Larman 在其著作中提出的一套原則，旨在指導開發者如何有效地**分配職責**（Responsibility Assignment）給物件。它是物件導向設計中極為關鍵的思維工具，共有九個核心模式：

### 1️⃣ Information Expert (資訊開發者)
*   **原則**：將職責分配給**擁有履行該職責所需資訊**的類別。
*   **優點**：資訊與行為封裝在一起，維持高內聚力。

### 2️⃣ Creator (建立者)
*   **原則**：如果類別 B 包含、聚合、記錄 A，或擁有 A 的初始化資料，則由 B 負責建立 A 的實例。
*   **優點**：減少整體系統的耦合。

### 3️⃣ Controller (控制器)
*   **原則**：分配一個類別（非 UI 類別）來負責接收並協調系統事件。
*   **優點**：將介面層與邏輯層分離，增加邏輯的可重用性。

### 4️⃣ Low Coupling (低耦合力)
*   **原則**：分配職責時，確保類別間的相依程度最低。（詳見 [7.2 節](#72-藕斷絲連耦合力與內聚力)）

### 5️⃣ High Cohesion (高內聚力)
*   **原則**：確保類別的功能是高度相關且集中的。（詳見 [7.2 節](#72-藕斷絲連耦合力與內聚力)）

### 6️⃣ Polymorphism (多型)
*   **原則**：當行為隨著「類型」改變時，使用多型操作而非顯式的條件判斷（如 `if-else`）。
*   **優點**：易於擴充新類型而不影響現有代碼。

### 7️⃣ Pure Fabrication (純虛構)
*   **原則**：當「資訊開發者」的設計會破壞內聚力時，建立一個原本不存在於領域模型的虛構類別（如 `DatabaseConnection`）來分擔職責。
*   **優點**：維持領域物件的純粹性與高內聚。

### 8️⃣ Indirection (間接接合)
*   **原則**：在兩個元件之間增加一個中介物件來解耦，避免兩者直接交談。
*   **優點**：降低兩者之間的變動影響，與「最少知識原則」相呼應。

### 9️⃣ Protected Variations (防護變異)
*   **原則**：找出預測的變動點，並在其外部封裝一個穩定的介面（防火牆）。（詳見 [7.7 節](#77-防火牆防護變異原則)）
...
本章節介紹了軟體設計的核心思考架構：
* **基本原則**：模組化設計、低耦高聚、不重複原則 (DRY)、資訊隱藏等。
* **進階防護**：開閉原則 (OCP) 與防護變異 (PV)，透過抽象化來隔離變化。
* **職責分配 (GRASP)**：提供了一套實踐框架，幫助我們決定「哪個功能該寫在哪個類別」。
