...
則日後 `Application` 物件想建立不同型態的文件物件 (例如 `HTMLDocument`、`WordDocument`)時，則必須修改 `operation1()` 方法如下：
...
[src/FactoryMethodTemplate.java](src/FactoryMethodTemplate.java)
...
[src/MazeGameSample.java](src/MazeGameSample.java) (見 `MazeGameLegacy` 類別)
...
[src/MazeGameSample.java](src/MazeGameSample.java) (見 `MazeGame` 類別)
...
[src/MazeGameSample.java](src/MazeGameSample.java) (見 `EnchantedMazeGame` 類別)
...
**靜態工廠方法 (Static Factory Method)** 是類別中一個公共的、靜態的方法，其目的是返回該類別的一個實例。與傳統的 `public constructor` 不同，它不是透過 `new` 關鍵字直接調用，而是透過類別名稱來調用。

#### 如何使用？

假設我們有一個 `User` 類別，傳統方式是使用 `constructor`：
...
User guest = User.createGuest("Bob");
```
...
2️⃣ **下方程式碼中，程式 `m2()` 是一個 factory method, 下列何者正確？（選二）**
- (A) `?1` 為 `A`
- (B) `?1` 為 `B`
...
5️⃣ **以下程式中，哪一個方法可能是 `factory method`?**
...
在考試系統中，可能進行各種不同的考試，例如小考 (`Quiz`)、正式考試 (`Exam`)、隨堂練習 (`Practice`) 等。`AbstractExamApp` 這個物件會產生考試物件，並開始進行一連串的活動（產生考卷、進行考試、閱卷）。請應用 `Factory Method` 來協助設計，並且降低 `ExamApp` 與考試類型之間的耦合力。

**提示：** 你需要設計 `AbstractExamApp`、`QuizApp`, `ExamApp`, `PracticeApp`, `AbstractTest`, `Quiz`, `Exam`, `Practice` 等類別。
...
在象棋系統中，可以如何應用 `Factory Method`? 請畫出 `UML` 類別圖並說明你的設計意圖。
