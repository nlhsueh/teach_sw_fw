...
4. 在 `ChessBoardView` 中，實作符合 `Consumer` 規格的 `updateView(ChessGameModel model)` 畫面渲染方法。
5. 在主程式的建構子中，使用方法參照（Method Reference，如 `view::updateView`）將 View 的更新邏輯註冊到 Model 中。
